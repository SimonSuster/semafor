...
public class IdFeatureExtractor {
	...
	public static IdFeatureExtractor fromName(String name) {
		final Map<String, Supplier<IdFeatureExtractor>> featureExtractorMap = ImmutableMap.of(
				"basic", new Supplier<IdFeatureExtractor>() {
					public IdFeatureExtractor get() { return new IdFeatureExtractor(false, false, false, false); }
				} ,
				"ancestor", new Supplier<IdFeatureExtractor>() {
					public IdFeatureExtractor get() { return new IdFeatureExtractor(false, true, false, false); }
				},
				"senna", new Supplier<IdFeatureExtractor>() {
					public IdFeatureExtractor get() { return new IdFeatureExtractor(false, false, true, false); }
				},
				"sennasyn", new Supplier<IdFeatureExtractor>() {
	...
	public static IdFeatureExtractor fromName(String name, int sennaVectorDim) {
		final Map<String, Supplier<IdFeatureExtractor>> featureExtractorMap = ImmutableMap.of(
				"senna", new Supplier<IdFeatureExtractor>() {
					public IdFeatureExtractor get() { return new IdFeatureExtractor(false, false, true, false); }
				},
	...
	public Map<String, Double> getBaseFeatures(int[] targetTokenIdxs, Sentence sentence) {
		...
		if (sennaSynFeatureExtractor.isPresent()) {
			results.putAll(sennaSynFeatureExtractor.get().getSennaFeatures(targetTokenIdxs, sentence));
		}
	...
	protected Map<String, Double> getSyntacticFeatures(int[] targetTokenIdxs, Sentence sentence) {
		...
		//(simon) deplabel of head
		final String depLabel = head.getLabelType().toUpperCase();
